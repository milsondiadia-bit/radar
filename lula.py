...
def classificar(grupos, chave_api):
    ...
    try:
        ...
        if m:
            try:
                ...
                pass
    if dado is None:
        # Resposta truncada: aproveita os objetos que vieram inteiros.
        pedacos = re.findall(r"\{[^{}]*\}", txt, re.S)
        recuperados = []
        for p in pedacos:
            try:
    ...
    # A IA pode devolver a lista solta ou embrulhada num objeto.
    ...
    if not isinstance(dado, list):
        print(f"Formato inesperado. Resposta crua:\n{txt[:800]}", file=sys.stderr)
        return {}
    ...
    for pos, o in enumerate(dado, 1):
        ...
        saida[n] = o
    if not saida:
        print(f"Nenhum item valido. Resposta crua:\n{txt[:800]}", file=sys.stderr)
    return saida
...
def main():
    ...
    grupos = grupos[:25]          # teto para nao estourar o prompt
